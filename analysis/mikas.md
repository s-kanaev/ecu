...
| ------------------------ | ------------ | ------ | -------- |
| `P0` `P2`                | Logic        | In/Out | Режим управляется битом `PMOD` в `SYSCON`: `0` = quasi-bidirectional port structure, `1` = bidirectional port structure. Предстоит ещё перевести сие на свой пигмейский). Однако, могут использоваться для передачи, соответственно, младшего и старшего байтов адреса для доступа к внешней памяти (FLASH). |
