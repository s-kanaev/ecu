...
Первый вариант не может занимать много команд. Второй варианд должен занимать много команд. При частоте вращения коленвала `6.5k` прерывание должно происходить с частотой `390k` (`300k` для частоты вращения КВ `5k`). При частоте работы CPU `16M` большая часть инструкций (`58%`) выполняется за `375nsec`. За одну секунду можно выпольнить `2.66M` команд.
